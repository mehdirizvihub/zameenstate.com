...
if(isset($_POST['update']))
{
	...
	$sql = "UPDATE advertisement SET title = '{$title}' , content = '{$content}', image ='{$aimage}' WHERE id = {$aid}";
	$result=mysqli_query($con,$sql);
	if($result == true)
	{
		$msg="<p class='alert alert-success'>advertisement Updated</p>";
		header("Location:advertisementview.php?msg=$msg");
	}
	else{
		$msg="<p class='alert alert-warning'>advertisement Not Updated</p>";
		header("Location:advertisementview.php?msg=$msg");
	}
}
?>
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header">
									<h2 class="card-title">Advertisement</h2>
								</div>
								<?php 
								$aid = $_GET['id'];
								$sql = "SELECT * FROM advertisement where id = {$aid}";
								$result = mysqli_query($con, $sql);
